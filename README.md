...
<a href="http://www.youtube.com/watch?feature=player_embedded&v=BvmEs0DE5Mk" target="_blank"><img align="right" src="http://img.youtube.com/vi/BvmEs0DE5Mk/0.jpg" alt="pynvme: test NVMe devices in Python" width="560" border="0" /></a>
...
```
Buffer.data(self, byte_end, byte_begin=None, type=int)
...
```
Controller.abort(self, cid, sqid=0, cb=None)
...
```
Controller.cmdlog(self, count=0)
...
```
Controller.downfw(self, filename, slot=0, action=1)
...
```
Controller.dst(self, stc=1, nsid=0xffffffff, cb=None)
...
```
Controller.format(self, lbaf=0, ses=0, nsid=1, cb=None)
...
```
Controller.fw_commit(self, slot, action, cb=None)
...
```
Controller.fw_download(self, buf, offset, size=None, cb=None)
...
```
Controller.getfeatures(self, fid, cdw11=0, cdw12=0, cdw13=0, cdw14=0, cdw15=0, sel=0, buf=None, cb=None)
...
```
Controller.getlogpage(self, lid, buf, size=None, offset=0, nsid=0xffffffff, cb=None)
...
```
Controller.id_data(self, byte_end, byte_begin=None, type=int, nsid=0, cns=1)
...
```
Controller.identify(self, buf, nsid=0, cns=1, cb=None)
...
```
Controller.sanitize(self, option=2, pattern=0, cb=None)
...
```
Controller.send_cmd(self, opcode, buf=None, nsid=0, cdw10=0, cdw11=0, cdw12=0, cdw13=0, cdw14=0, cdw15=0, cb=None)
...
```
Controller.setfeatures(self, fid, cdw11=0, cdw12=0, cdw13=0, cdw14=0, cdw15=0, sv=0, buf=None, cb=None)
...
```
Controller.waitdone(self, expected=1)
...
```
Namespace.compare(self, qpair, buf, lba, lba_count=1, io_flags=0, cb=None)
compare IO command
...
```
Namespace.dsm(self, qpair, buf, range_count, attribute=0x4, cb=None)
...
```
Namespace.flush(self, qpair, cb=None)
flush IO command
...
```
Namespace.get_lba_format(self, data_size=512, meta_size=0)
...
```
Namespace.id_data(self, byte_end, byte_begin=None, type=int)
...
```
Namespace.ioworker(self, io_size, lba_align, lba_random, read_percentage, time=0, qdepth=64, region_start=0, region_end=0xffffffffffffffff, iops=0, io_count=0, lba_start=0, qprio=0, output_io_per_second=None, output_percentile_latency=None)
...
```
Namespace.read(self, qpair, buf, lba, lba_count=1, io_flags=0, cb=None)
read IO command
...
```
Namespace.send_cmd(self, opcode, qpair, buf=None, nsid=0, cdw10=0, cdw11=0, cdw12=0, cdw13=0, cdw14=0, cdw15=0, cb=None)
...
```
Namespace.write(self, qpair, buf, lba, lba_count=1, io_flags=0, cb=None)
write IO command
...
```
Namespace.write_uncorrectable(self, qpair, lba, lba_count=1, cb=None)
...
```
Namespace.write_zeroes(self, qpair, lba, lba_count=1, io_flags=0, cb=None)
...
### msix_clear
```python
Qpair.msix_clear(self)
```
Qpair.msix_clear(self)
### msix_isset
```python
Qpair.msix_isset(self)
```
Qpair.msix_isset(self)
### msix_mask
```python
Qpair.msix_mask(self)
```
Qpair.msix_mask(self)
### msix_unmask
```python
Qpair.msix_unmask(self)
```
Qpair.msix_unmask(self)
### waitdone
```python
...
```
Qpair.waitdone(self, expected=1)
sync until expected commands completion
...
```
Subsystem.power_cycle(self, sec=10)
...
```
Subsystem.shutdown_notify(self, abrupt=False)
